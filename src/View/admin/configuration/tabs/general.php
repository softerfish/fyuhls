<div class="config-section-shell">
    <div class="config-section-nav">
        ...
        <div class="nav flex-column nav-pills">
            ...
            <a class="nav-link text-start" href="#general-video"><i class="bi bi-camera-video me-2"></i> Video Processing</a>
        </div>
    </div>
    <div class="config-section-content">
        <div class="config-section-intro">
            <div>
                ...
                <p class="config-section-intro__text">Manage the core site identity, member-facing platform switches, and optional server-side video tooling that powers thumbnails and transcoding.</p>
            </div>
        </div>
        <details class="config-help-panel">
            <summary>How this works</summary>
            <div class="config-help-panel__body">
                <p>These are the install-wide defaults that define what the site is called, whether new users can sign up, and whether extra infrastructure like FFmpeg is available for media workflows.</p>
            </div>
        </details>
    ...
    <?php renderAdminCardStart('Platform Controls', ['cardClass' => 'border-0 shadow-sm mb-4 config-section-card']); ?>
        ...
        <div class="col-md-6">
            ...
            <div class="mb-3">
                ...
                <small class="config-form-note text-danger">Enables demo-mode behavior for the site and its designated demo admin account. Use this together with the demo-admin assignment in Users when you want one admin account to stay redacted and read-only while other admins keep normal access. Default: Off.</small>
            </div>
            </div>
        </div>
    <?php renderAdminCardEnd(); ?>

    <div id="general-video"></div>
    <?php renderAdminCardStart('Video Processing', ['cardClass' => 'border-0 shadow-sm mb-4 config-section-card']); ?>
            <div class="row">
            ...
            <small class="config-form-note">Requires FFmpeg binary to be installed on your server.</small>
            </div>
            ...
            <small class="config-form-note">Full path to the ffmpeg executable.</small>
            </div>
            </div>
    <?php renderAdminCardEnd(); ?>

    <div class="config-sticky-save">
        <p class="config-sticky-save__text">General settings control the install-wide defaults most users feel first, so changes here are usually visible immediately.</p>
