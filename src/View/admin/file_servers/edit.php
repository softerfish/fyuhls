                ...
                <form method="POST">
                    ...
                    <?php if ($activeTab === 'wasabi'): ?>
                        <div class="mb-3">
                            ...
                            <input type="text" name="path" id="wasabiBucketName" class="form-control" value="<?= htmlspecialchars($server['storage_path']) ?>" required>
                        </div>
                        <div class="row g-3 mb-3">
                            <div class="col-md-6">
                                <label class="form-label fw-bold small">Wasabi Region</label>
                                <input type="text" name="config[s3_region]" id="wasabiRegion" class="form-control" value="<?= htmlspecialchars($config['s3_region'] ?? '') ?>" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="password" name="config[s3_key]" id="wasabiAccessKey" class="form-control" value="" placeholder="<?= htmlspecialchars($hiddenKeyPlaceholder) ?>" autocomplete="off" spellcheck="false">
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold small">Wasabi Endpoint Host or URL (Optional)</label>
                            <input type="text" name="config[s3_endpoint]" id="wasabiEndpoint" class="form-control" value="<?= htmlspecialchars($config['s3_endpoint'] ?? '') ?>" placeholder="e.g. s3.us-east-1.wasabisys.com">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="password" name="config[s3_secret]" id="wasabiSecretKey" class="form-control" value="" placeholder="<?= htmlspecialchars($hiddenKeyPlaceholder) ?>" autocomplete="off" spellcheck="false">
                            <div class="text-muted extra-small mt-1">Re-enter the current Wasabi Access Key and Secret Key here if you want Fyuhls to load buckets or update the real bucket CORS rule on your behalf.</div>
                        </div>
                        <div class="border rounded p-3 bg-light mb-4">
                            <div class="d-flex flex-wrap gap-2 mb-3">
...
Invoke-RestMethod -Method Post -Uri ($apiUrl + "/b2api/v3/b2_update_bucket") -Headers @{ Authorization = $auth.authorizationToken } -Body $updateBody -ContentType "application/json"</code></pre>
                ...
                <?php if ($activeTab === 'wasabi'): ?>
                    <div class="bg-white border rounded p-3 small mt-3">
                        <div class="fw-bold mb-2">Wasabi edit checklist</div>
                        <ol class="mb-0 ps-3">
                            <li>Confirm the bucket name still matches the same Wasabi bucket that already holds your files.</li>
                            <li>Confirm the region still matches the real endpoint, such as <code>us-east-1</code> from <code>s3.us-east-1.wasabisys.com</code>.</li>
                            <li>If you rotated your Wasabi keys, update the access key here and paste the new secret key.</li>
                            <li>When creating a replacement key in Wasabi, go to <strong>Access Keys</strong>, click <strong>Create Access Key</strong>, and choose <strong>Sub-User</strong> if possible instead of <strong>Root User</strong>.</li>
                            <li>Leave the Wasabi endpoint field blank unless you already know the exact Wasabi S3 endpoint. Fyuhls can build it from the region as <code>https://s3.your-region.wasabisys.com</code>.</li>
                            <li>Leave the secret blank only if you want to keep the currently saved one.</li>
                            <li>If browser uploads fail, re-check the bucket's real CORS rules and confirm they still allow your exact site origin, <code>PUT</code>, <code>GET</code>, and <code>HEAD</code>, and expose <code>ETag</code>.</li>
                        </ol>
                    </div>
                    <div class="alert alert-secondary border-0 small mt-3">
                        <strong>Wasabi key reminder:</strong> The Wasabi console shows the <strong>Secret Key</strong> only once when the key is created. Copy it immediately when rotating credentials.
                    </div>
                    <div class="alert alert-light border small mt-3">
                        <strong>Endpoint field tip:</strong> Only fill in <strong>Wasabi Endpoint Host or URL</strong> if you want to paste the exact endpoint yourself, such as <code>https://s3.us-east-1.wasabisys.com</code> or <code>s3.us-east-1.wasabisys.com</code>. Most installs should leave it blank and just keep the region correct.
                    </div>
                    <div class="bg-white border rounded p-3 small mt-3">
                        <div class="fw-bold mb-2">Recommended S3-compatible CORS rule</div>
<pre class="mb-2"><code>&lt;CORSConfiguration&gt;
...
&lt;/CORSConfiguration&gt;</code></pre>
                    ...
                    <div class="alert alert-secondary border-0 small mt-3">
                        <strong>Where this goes:</strong> This rule is configured on the <strong>Wasabi bucket itself</strong>, not in Fyuhls. If the Wasabi dashboard does not expose the full S3 CORS options you need, use the provider's S3-compatible API or CLI instead.
                    </div>
                <?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    discoverBtn.addEventListener('click', async function () {
        ...
        if (!accessKeyInput.value.trim() || !secretKeyInput.value.trim()) {
            setStatus('Re-enter the current Wasabi Access Key and Secret Key first. Fyuhls cannot reload buckets from Wasabi without both values.', 'warning');
            return;
        }

        setBusy(true);
        setStatus('Connecting to Wasabi and reloading your buckets...', 'info');
        ...
        try {
            ...
            if ((result.buckets || []).length === 0) {
                ...
                setStatus('Wasabi connected. Pick a bucket if needed, and Fyuhls will keep the bucket, region, and endpoint fields in sync.', 'success');
            }
        } catch (error) {
            bucketPickerWrap.classList.add('d-none');
    ...
    applyCorsBtn.addEventListener('click', async function () {
        ...
        if (!accessKeyInput.value.trim() || !secretKeyInput.value.trim() || !bucketName) {
            setStatus('Re-enter the Access Key and Secret Key and make sure the bucket name is filled in before applying the Fyuhls CORS rule.', 'warning');
            return;
        }
